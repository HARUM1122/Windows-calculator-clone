...
String _convert(String n, List<Enum> units, int from, int to) {
  ...
  return addCommas(
      standardToScientific(trimTrailingZeros(convertedValue.toString())));
}
...
class DialogProvider extends ChangeNotifier {
  void changeUnit(int index, {bool second = false}) {
    ...
    if (selectedTextIndex == 0) {
      secondText =
          _convert(firstText, unitList, selectedUnitIndex, selectedUnitIndex2);
    } else {
      firstText =
          _convert(secondText, unitList, selectedUnitIndex2, selectedUnitIndex);
    }
...
class UnitProvider extends ChangeNotifier {
  ...
  void del() {
    ...
    currentText = currentText.length == 1 ||
            (currentText.startsWith("-") && currentText.length == 2)
        ? "0"
        : addCommas(currentText.substring(0, currentText.length - 1));
    if (!currentText.endsWith(".")) {
      otherText = _convert(
          currentText,
          unitList,
