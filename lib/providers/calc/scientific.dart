...
class ScientificCalcProvider extends ChangeNotifier {
  ...
  String evaluateExpression(String exp, {bool ignoreErrors = false}) {
    ...
    try {
      String ans =
          trimTrailingZeros(Expression(exp).eval()!.toStringAsFixed(12));
      return ans;
    } on ExpressionException catch (e) {
      throwError(
          msg: e.msg == 'Overflow' || e.msg == 'Cannot divide by 0'
              ? e.msg
              : 'Invalid input');
      return 'E';
    }
  ...
  void addToScreen(String value) {
    ...
    if (screenText.contains('e')) {
      if ((screenText.contains('e+') &&
              screenText.split('e+').last.length >= 2) ||
          (screenText.contains('e-') &&
              screenText.split('e-').last.length >= 2)) {
        return;
      }
    ...
    if ((screenText.contains('.') && value == '.') ||
        screenText.replaceAll('.', '').replaceAll(',', '').length == 20) {
      return;
    }
  ...
  void posNeg() {
    ...
    if (screenText.contains('e')) {
      ...
      screenText =
          screenText.startsWith('-') ? screenText.substring(1) : '-$screenText';
    }
  ...
  void useParenthesis(String parenthesis) {
    ...
    if (parenthesis == "(" && parenthesesCount != 25) {
      if (_isDigit(_getLast(expression)) || expression.endsWith(")")) {
        ...
        expression +=
            "${expression.endsWith('(') || expression.isEmpty ? '' : ' '}$parenthesis";
      }
      final int lastOpeningParenthesis =
          _getLastMostOpeningParenthesis(expression);
  ...
  void useOperator(String op) {
    ...
    if (!opUsed) {
      ...
      if (expression.endsWith(")")) {
        ...
        expression +=
            "${expression.endsWith('(') || expression.isEmpty ? '' : ' '}$sc $op";
      }
      final String ans = evaluateExpression(parseExpression(expression));
      if (ans == "E") {
        return;
  ...
  void useFunc(String func) {
    ...
    if (expression.endsWith(")")) {
      ...
      expression +=
          "${expression.endsWith('(') || expression.isEmpty ? '' : ' '}$func";
    }
    final int lastOpeningParenthesis =
        _getLastMostOpeningParenthesis(expression);
    openingParenthesesStack.removeLast();
  ...
  void eql(Function? add) {
    ...
    screenText = standardToScientific(trimTrailingZeros(screenText),
        pos: 1e20, neg: -1e20);
    if (calculated) {
      final List<String> exp = expression.split(' ');
      ...
      if ((typed || opUsed || !_containDigits(expression)) &&
          parenthesesCount == 0) {
        if (expression.endsWith(")")) {
