...
class DateCalcProvider extends ChangeNotifier{
  ...
  String getDifference(int ms){
    ...
    if(months>0){
      result+="${result.isEmpty?'':', '} $months ${months!=1?'months':'month'}";
    }
    if(weeks>0){
      result+="${result.isEmpty?'':', '} $weeks ${weeks!=1?'weeks':'week'}";
    }
    if(days>0){
      result+="${result.isEmpty?'':', '} $days ${days!=1?'days':'day'}";
    }
    return result;
  }
